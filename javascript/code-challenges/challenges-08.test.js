...
const sortByChildren = (charArray) => {
  ...
  return charArray.sort((a, b) => {
    let numKids = a.children.length - b.children.length;
    if (numKids === 0) {
...
let numberedHouse = sortByChildren(characters);
console.log(numberedHouse);
...
const isNum = (input) => {
  // Solution code here...
  let findNum = /\d/;
  let makeString = typeof input === 'number' ? input.toString() : input;

  return findNum.test(makeString);
};
...
let string = 'We only want to Return the Words that begin With a capital Letter';
let stringTwo = 'Given by our hand in the meadow that is called Runnymede, between Windsor and Staines, on the fifteenth day of June in the seventeenth year of our reign (i.e. 1215: the new regnal year began on 28 May).'
let stringThree ='these words are all failures';

console.log(isCapitalized(string));
console.log(isCapitalized(stringTwo));
console.log(isCapitalized(stringThree));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let findCity= /^[A-J]/i;
  return arr.filter(city => findCity.test(city));
};

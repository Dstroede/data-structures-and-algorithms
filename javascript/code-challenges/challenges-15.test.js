...
const screenForNames = (arr) => {
  // Solution code here...
  let nameReg = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-za-z]+(\s[A-Za-z]+)?$/;

  return arr.filter((name)=> nameReg.test(name));
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let lukeCharacter = arr.find((char) => char.name.toLowerCase() === 'luke skywalker');
  let lukeMass = lukeCharacter ? Number(lukeCharacter.mass) : 0;
  let names = arr
    .filter((char) => char.mass !== 'unknown' && Number(char.mass) > lukeMass)
    .map((char) => char.name);
  return names.join(' - ');
};
...
let secureNot= 'http://www.insecure.com returns false because the URL is not secure'
let secureYes= 'https://secure.com returns true because the URL is secure'
let secureMalformed= 'https:/missingslash.org returns false because the URL is malformed'
console.log(isSecure(secureNot));
console.log(isSecure(secureYes));
console.log(isSecure(secureMalformed));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let checkBoard = (row1, col1, row2, col2, row3, col3) => {
    return board[row1][col1] !== '' &&
           board[row1][col1] === board[row2][col2] &&
           board[row1][col1] === board[row3][col3];
  };
  for (let row = 0; row < 3; row++) {
    if(checkBoard(row, 0, row, 1, row, 2)) {
      return true;
    }
  }

  for (let col = 0; col < 3; col++) {
    if(checkBoard(col, 0, col, 1, col, 2)) {
      return true;
    }
  }

  if(checkBoard(0, 0, 1, 1, 2, 2) || checkBoard(0, 2, 1, 1, 2, 0)){
    return true;
  }

...
const toLastNames = people => {
  // Solution code here...
  let fullName = people.map(a => a.firstName +' '+ a.lastName);
  return fullName;
};
const people = [{ firstName: 'Jane', lastName: 'Doe' }, { firstName: 'James', lastName: 'Bond' }];
let fullNameArray= toLastNames(people);
...
const addPurchases = (itemArr) => {
  // Solution code here...
  let total = itemArr.reduce((a,b) => {
    return a + b.purchasePrice;
  }, 0);
  return total;
};
let itemArr = [{item: 'switch', purchasePrice: 399}, {item: 'toothpaste', purchasePrice: 2}];
let totalPrice = addPurchases(itemArr);
...
const countNumberOfElements = (countArr) => {
  // Solution code here...
  return countArr.reduce((a,_) => a + 1, 0);
};
let countArr= [1, 2, 3, 4, 5];
let totalCount = countNumberOfElements(countArr);
...
const returnNames = (starWarsData) => {
  // Solution code here...
  let findName = starWarsData.reduce((a, b)=>{
    a.push(b.name);
    return a;
  }, []);
  return findName;
};
let showNames = returnNames(starWarsData);
...
const reversedString = (revStr) => {
  // Solution code here...
  return revStr.split('').reduce((a, b)=>{
    return b+ a;
  }, '');
};
let revStr= 'Code 301';
let stringRev = reversedString(revStr);

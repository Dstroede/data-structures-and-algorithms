...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  starWarsArr.forEach(obj => {
    obj.height = obj.height.toString();
  });
...
let thisArr = [1, 2, 3, 4, 5, 6, 7, 8];
removeThree(3, thisArr);
console.log(thisArr);
...
let threeArr = ['hello', '301', 'students'];
let newString = joinArray(threeArr);
...
let resultArray= howMuchPencil(welcomeArr[0]);
...
let breakdown = ('Gregor');
let newWords = wordsToCharList(breakdown);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(food => {
    let firstSpaceIndex = food.indexOf(' ');
    if (firstSpaceIndex !== -1) {
      let secSpaceIndex = food.indexOf(' ', firstSpaceIndex + 1);
      let foodFound = secSpaceIndex !== -1 ? food.slice(secSpaceIndex + 1) : food;
      result.push(foodFound);
    } else {
      result.push(food);
    }
  });
...
let groceryList = listFoods(gruffaloCrumble);

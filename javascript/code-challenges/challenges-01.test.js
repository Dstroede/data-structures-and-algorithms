...
let arrayTwo = ["HI", "HOW", "ARE", "YOU"];
let exArray= addExclamation(arrayTwo);
console.log(exArray);
...
const allUpperCase = (arr) => {
  let newArr = [];
  arr.forEach( (a) => {
    let cap = a.toUpperCase();
    newArr.push(cap);
  });
  return newArr;
};
let arrayThree = ['hi', 'how', 'are', 'you'];
let capArray= allUpperCase(arrayThree);
...
const speaker = (words, callback) => {
  let newWords = [];
  words.forEach( a => {
    let together = callback(a);
    newWords.push(together);
  });
  return newWords;
};
let startArray = ['hello', '301', 'Class'];
let max= speaker(startArray, greeting);
console.log(max);
